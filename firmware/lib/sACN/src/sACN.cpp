...
{
	// verify root layer
	if (sacnPacket[PREAMBLE_ADDR] != PREAMBLE[0])
		return false;
	if (sacnPacket[PREAMBLE_ADDR + 1] != PREAMBLE[1])
		return false;
	if (sacnPacket[POSTAMBLE_ADDR] != POSTAMBLE[0])
		return false;
	if (sacnPacket[POSTAMBLE_ADDR + 1] != POSTAMBLE[1])
		return false;
	for (uint8_t i = 0; i < ACN_IDENTIFIER_SIZE; i++)
	{
		if (sacnPacket[i + ACN_IDENTIFIER_ADDR] != ACN_IDENTIFIER[i])
			return false;
	}
	...
	if (packetSize != rootFlagAndLength)
		return false;
	for (uint8_t i = 0; i < VECTOR_ROOT_E131_DATA_SIZE; i++)
	{
		if (sacnPacket[i + VECTOR_ROOT_E131_DATA_ADDR] != VECTOR_ROOT_E131_DATA[i])
			return false;
	}
	...
	if (packetSize != framingFlagAndLength)
		return false;
	for (uint8_t i = 0; i < VECTOR_E131_DATA_PACKET_SIZE; i++)
	{
		if (sacnPacket[i + VECTOR_E131_DATA_PACKET_ADDR] != VECTOR_E131_DATA_PACKET[i])
			return false;
	}
	...
	if (universe != ((sacnPacket[UNIVERSE_ADDR] << 8) + sacnPacket[UNIVERSE_ADDR + 1]))
		return false;
	...
	if (packetSize != dmpFlagAndLength)
		return false;
	if (sacnPacket[VECTOR_DMP_SET_PROPERTY_ADDR] != VECTOR_DMP_SET_PROPERTY)
		return false;
	if (sacnPacket[DMP_ADDRESS_AND_DATA_ADDR] != DMP_ADDRESS_AND_DATA)
		return false;
	if (sacnPacket[FIRST_PROPERTY_ADDRESS_ADDR] != FIRST_PROPERTY_ADDRESS[0])
		return false;
	if (sacnPacket[FIRST_PROPERTY_ADDRESS_ADDR + 1] != FIRST_PROPERTY_ADDRESS[1])
		return false;
	if (sacnPacket[ADDRESS_INC_ADDR] != ADDRESS_INC[0])
		return false;
	if (sacnPacket[ADDRESS_INC_ADDR + 1] != ADDRESS_INC[1])
		return false;
	propertyValueCount = (sacnPacket[PROPERTY_VALUE_COUNT_ADDR] << 8) + sacnPacket[PROPERTY_VALUE_COUNT_ADDR + 1];
	if ((packetSize - STARTCODE_ADDR) != propertyValueCount)
		return false;
	if (sacnPacket[STARTCODE_ADDR] != STARTCODE_DMX)
		return false;
	...
	{
		source.seqDiff = seqNumber - source.seqNumber;
		callSeqDiffFunction();
	}
	...
	else
	{
		...
		source.frameRateTimestamp = millis();
		if (callFramerateFunction != NULL)
			callFramerateFunction();
	}
...
void Receiver::callbackSeqDiff(fptr callSeqDiff)
{
	callSeqDiffFunction = callSeqDiff;
}
...
void Source::stop()
{
	...
	if (priorityDD)
		sacnPacketDD[OPTIONS_ADDR] = STREAM_TERMINATED;
	for (uint8_t i = 0; i < 3; i++)
	{
		send();
		if (priorityDD)
...
{
	if (unicastMode)
		udp->beginPacket(ip, ACN_SDT_MULTICAST_PORT);
	else
		udp->beginPacket(mcastIP, ACN_SDT_MULTICAST_PORT);
